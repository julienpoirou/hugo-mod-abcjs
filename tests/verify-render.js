...
async function main() {
  ...
  try {
    ...
    await page.waitForSelector('[data-hugo-mod-abcjs][data-rendered="true"]', { timeout: 15000 });

    const svgCount = await page.locator("[data-hugo-mod-abcjs] svg").count();
    ...
    if (svgCount < 1) {
      console.error("FAIL: no <svg> rendered inside the abcjs wrapper");
      process.exit(1);
    }
    ...
    console.log(`PASS: ${svgCount} abcjs score(s) rendered to real <svg>, no errors, no failed requests`);
  } finally {
    await browser.close();
    server.close();
  }
